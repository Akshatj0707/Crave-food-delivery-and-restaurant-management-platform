...
const STATUS_LABELS = { confirmed: 'Order Confirmed', preparing: 'Preparing', ready: 'Ready', out_for_delivery: 'Out for Delivery', delivered: 'Delivered' };
    ...
    <div style={{ minHeight: '100vh', background: 'var(--gray-50)' }}>
      ...
      <div className="container" style={{ padding: '40px 20px', maxWidth: 720 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
          <button onClick={() => navigate('/orders')} style={{ background: 'white', border: '1px solid var(--gray-200)', borderRadius: 10, padding: '8px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, fontSize: 14 }}>
            <ChevronLeft size={16} /> Back
          </button>
          <div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 900 }}>Order #{order.order_number}</h1>
                ...
                return (
                  <React.Fragment key={step}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: i < STATUS_STEPS.length - 1 ? 'none' : 1 }}>
                      ...
                      <p style={{ fontSize: 11, fontWeight: done ? 700 : 400, color: done ? 'var(--crave-orange)' : 'var(--gray-400)', marginTop: 8, textAlign: 'center', maxWidth: 70 }}>
                        {STATUS_LABELS[step]}
                      </p>
                    </div>
                    {i < STATUS_STEPS.length - 1 && (
                      <div style={{ flex: 1, height: 3, background: i < currentStepIdx ? 'var(--crave-orange)' : 'var(--gray-200)', transition: 'var(--transition)', marginBottom: 24 }} />
                    )}
                  </React.Fragment>
                );
              })}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
          <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 20, boxShadow: 'var(--shadow-sm)', border: '1px solid var(--gray-200)' }}>
            ...
            <p style={{ fontWeight: 800, fontSize: 16, marginBottom: 4 }}>{order.restaurant_name}</p>
            <p style={{ fontSize: 13, color: 'var(--gray-500)', display: 'flex', alignItems: 'center', gap: 4 }}>
              <Phone size={12} /> {order.restaurant_phone}
            </p>
          </div>
          {order.service_mode === 'delivery' && order.address_line1 && (
            <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 20, boxShadow: 'var(--shadow-sm)', border: '1px solid var(--gray-200)' }}>
        ...
        <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 24, marginBottom: 24, boxShadow: 'var(--shadow-sm)', border: '1px solid var(--gray-200)' }}>
          ...
          {order.items?.map((item, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: i < order.items.length - 1 ? '1px solid var(--gray-100)' : 'none' }}>
              ...
              <span style={{ fontWeight: 700 }}>₹{parseFloat(item.total || item.price * item.quantity).toFixed(2)}</span>
            </div>
          ))}
          ...
          <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1.5px solid var(--gray-100)', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              ...
              ...(parseFloat(order.delivery_fee) > 0 ? [['Delivery Fee', `₹${parseFloat(order.delivery_fee).toFixed(2)}`]] : [['Delivery', 'Free']]),
              ['GST', `₹${parseFloat(order.tax_amount).toFixed(2)}`],
              ...(parseFloat(order.discount_amount) > 0 ? [['Discount', `-₹${parseFloat(order.discount_amount).toFixed(2)}`]] : []),
            ].map(([l, v]) => (
        ...
        {order.status === 'delivered' && !reviewDone && (
          <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 24, boxShadow: 'var(--shadow-sm)', border: '1px solid var(--gray-200)' }}>
            ...
            <p style={{ fontSize: 14, color: 'var(--gray-500)', marginBottom: 20 }}>How was your food from {order.restaurant_name}?</p>
