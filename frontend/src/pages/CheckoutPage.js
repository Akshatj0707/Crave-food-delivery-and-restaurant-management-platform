...
import { MapPin, CreditCard, Smartphone, Building2, CheckCircle, Plus, Bike, Utensils, UtensilsCrossed } from 'lucide-react';
      ...
      if (payMethod === 'card') {
        const result = await stripe.confirmCardPayment(clientSecret, {
          payment_method: {
            ...
            billing_details: { name: order.customer_name }
          }
        });
        ...
        if (result.paymentIntent.status === 'succeeded') {
          await paymentAPI.confirm({ paymentIntentId: result.paymentIntent.id, orderId: order.id });
          onSuccess(order.id);
        }
      } else {
        // For UPI/Net Banking demo - simulate success
        toast.loading('Processing payment...', { duration: 2000 });
        await new Promise(r => setTimeout(r, 2000));
        await paymentAPI.confirm({ paymentIntentId: 'demo_pi', orderId: order.id }).catch(() => {});
        onSuccess(order.id);
      }
    } catch (err) {
    ...
    <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 24, boxShadow: 'var(--shadow-sm)', border: '1px solid var(--gray-200)' }}>
      ...
      {payMethod === 'card' && (
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--gray-500)', marginBottom: 16 }}>
            🔒 Test with card: 4242 4242 4242 4242 · Any future date · Any 3-digit CVC
          </p>
        </div>
      )}
      ...
      {payMethod === 'upi' && (
        <div style={{ marginBottom: 16 }}>
          <input className="form-input" placeholder="Enter UPI ID (e.g. name@upi)" />
          <p style={{ fontSize: 12, color: 'var(--gray-500)', marginTop: 6 }}>Demo mode: payment will be simulated</p>
        </div>
      )}
      ...
      <button
        onClick={handlePay}
        disabled={processing || (!stripe && payMethod === 'card')}
        className="btn btn-primary"
        style={{ width: '100%', fontSize: 16, padding: '16px' }}
      >
        {processing ? 'Processing...' : `Pay ₹${parseFloat(order.total_amount).toFixed(2)}`}
      </button>
    </div>
  );
}
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        subtotal, deliveryFee: DELIVERY_FEE, taxAmount: tax,
        discountAmount: 0, totalAmount: total
      };
      const res = await orderAPI.create(orderData);
      setOrder({ ...res.data.data, customer_name: `${user.first_name} ${user.last_name}`, total_amount: total });
    } catch (err) {
              ...
              <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 24, boxShadow: 'var(--shadow-sm)', border: '1px solid var(--gray-200)' }}>
                ...
                {addresses.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {addresses.map(addr => (
                      <label key={addr.id} style={{ display: 'flex', gap: 12, padding: '14px 16px', borderRadius: 10, cursor: 'pointer', border: selectedAddress === addr.id ? '2px solid var(--crave-orange)' : '1.5px solid var(--gray-200)', background: selectedAddress === addr.id ? 'var(--crave-orange-pale)' : 'white' }}>
                        <input type="radio" name="address" value={addr.id} checked={selectedAddress === addr.id} onChange={() => setSelectedAddress(addr.id)} style={{ accentColor: 'var(--crave-orange)', marginTop: 2 }} />
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 2 }}>{addr.label}</div>
                          <div style={{ fontSize: 13, color: 'var(--gray-600)' }}>{addr.address_line1}, {addr.city}, {addr.state} - {addr.pincode}</div>
                        </div>
                      </label>
                    ))}
                  </div>
            ...
            {!order && (
              <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 24, boxShadow: 'var(--shadow-sm)', border: '1px solid var(--gray-200)' }}>
                ...
                <textarea
                  ...
                  placeholder="Any special requests? (allergies, customizations, etc.)"
